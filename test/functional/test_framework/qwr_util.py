#!/usr/bin/env python3
# Copyright (c) 2014-2022 The Qweercoin Core developers
...
from test_framework.util import get_datadir_path, initialize_datadir, twinks_round
...
def create_non_hd_wallet(chain, options):    
    ...
    conf_file = os.path.join(data_dir, 'qweercoin.conf')
    ...
    v15_node = TestNode(
        ...
        timewait=60,
        timeout_factor=1.0,
        qweercoind=os.path.join(bin_dir, 'qweercoind'),
        qweercoin_cli=os.path.join(bin_dir, 'qweercoin-cli'),
        version=version,
        coverage_dir=None,
        cwd=options.tmpdir,
        extra_args=["-usehd=0"],
