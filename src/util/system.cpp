...
const char * const QWEERCOIN_CONF_FILENAME = "qweercoin.conf";
const char * const QWEERCOIN_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/qweercoin/qweercoin/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //qweercoin-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", QWEERCOIN_SETTINGS_FILENAME);
...
#else
    const char* pszModule = "qweercoin";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Qweercoin
    // macOS: ~/Library/Application Support/Qweercoin
    // Unix-like: ~/.qweercoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Qweercoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Qweercoin";
#else
    // Unix-like
    return pathRet / ".qweercoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", QWEERCOIN_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure Qweercoin Core copyright is not removed by accident
    if (copyright_devs.find("Qweercoin Core") == std::string::npos) {
        if (strPrefix.find("2011") != std::string::npos) {
            std::string strYear = strPrefix;
            strYear.replace(strYear.find("2011"), sizeof("2011")-1, "2009");
            strCopyrightHolders += "\n" + strYear + "The Qweercoin Core developers";
        }
    }
