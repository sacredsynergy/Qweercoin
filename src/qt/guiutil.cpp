...
#include <qt/qweercoinaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Qweercoin address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new QweercoinAddressEntryValidator(parent));
    widget->setCheckValidator(new QweercoinAddressCheckValidator(parent));
}

bool parseQweercoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no qweercoin: URI
    if(!uri.isValid() || uri.scheme() != QString("qweercoin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!QweercoinUnits::parse(QweercoinUnits::QWR, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseQweercoinURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseQweercoinURI(uriInstance, out);
}

QString formatQweercoinURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("qweercoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(QweercoinUnits::format(QweercoinUnits::QWR, info.amount, false, QweercoinUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openQweercoinConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", QWEERCOIN_CONF_FILENAME));
    ...
    /* Open qweercoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Qweercoin.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Qweercoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Qweercoin (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for Qweercoin*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "qweercoin.desktop";
    return GetAutostartDir() / strprintf("qweercoin-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a qweercoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Qweercoin\n";
        else
            optionFile << strprintf("Name=Qweercoin (%s)\n", chain);
