...
{
    GUIUtil::setClipboard(QweercoinUnits::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
...
void CoinControlDialog::updateLabels(CCoinControl& m_coin_control, WalletModel *model, QDialog* dialog)
{
    ...
    if (nQuantity > 0)
    {
        ...
        if (fWitness)
        {
            ...
            // usually, the result will be an overestimate within a couple of twinks so that the confirmation dialog ends up displaying a slightly smaller fee.
    ...
    int nDisplayUnit = QweercoinUnits::QWR;
    ...
    l2->setText(QweercoinUnits::formatWithUnit(nDisplayUnit, nAmount));        // Amount
    l3->setText(QweercoinUnits::formatWithUnit(nDisplayUnit, nPayFee));        // Fee
    l4->setText(QweercoinUnits::formatWithUnit(nDisplayUnit, nAfterFee));      // After Fee
    l5->setText(((nBytes > 0) ? ASYMP_UTF8 : "") + QString::number(nBytes));        // Bytes
    l7->setText(fDust ? tr("yes") : tr("no"));                               // Dust
    l8->setText(QweercoinUnits::formatWithUnit(nDisplayUnit, nChange));        // Change
    if (nPayFee > 0)
    {
    ...
    // how many twinks the estimated fee can vary per byte we guess wrong
    double dFeeVary = (nBytes != 0) ? (double)nPayFee / nBytes : 0;

    QString toolTip4 = tr("Can vary +/- %1 twinks(s) per input.").arg(dFeeVary);
...
void CoinControlDialog::updateView()
{
    ...
    for (const auto& coins : model->wallet().listCoins()) {
        ...
        for (const auto& outpair : coins.second) {
            ...
            // if listMode or change => show qweercoin address. In tree mode, address is not shown again for direct wallet address outputs
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, QweercoinUnits::format(nDisplayUnit, out.nValue));
        ...
        if (treeMode)
        {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, QweercoinUnits::format(nDisplayUnit, nSum));
