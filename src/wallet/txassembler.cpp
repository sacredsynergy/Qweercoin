    ...
    const bool sign)
{
    ...
    while (true) {
        ...
        // the recipients are funded through the pegout kernel instead of traditional QWR outputs.
        ...
        // Calculate transaction size and the total necessary fee amount (includes QWR and MWEB fees)
    ...
    if (new_tx.mweb_type != MWEB::TxType::QWR_TO_QWR) {
...
bool TxAssembler::AttemptCoinSelection(InProcessTx& new_tx, const CAmount& nTargetValue) const
{
    ...
    if (new_tx.recipients.front().IsMWEB()) {
        ...
        // First try to construct a QWR-to-QWR transaction
        CoinSelectionParams mweb_to_mweb = new_tx.coin_selection_params;
        mweb_to_mweb.input_preference = InputPreference::QWR_ONLY;
        ...
        // If QWR-to-QWR fails, create a peg-out transaction
