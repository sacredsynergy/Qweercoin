...
The Qweercoin Core daemon can be configured to act as a trusted "border
router", implementing the qweercoin wire protocol and relay, making
...
The ZeroMQ feature in Qweercoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building qweercoind:
...
For instance:
    ...
    $ qweercoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/qweercoind.tx.raw \
...
These options can also be provided in qweercoin.conf.
...
From the perspective of qweercoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into qweercoind directly. Furthermore, no information is
...
using. Qweercoind appends an up-counting sequence number to each
