Qweercoin Core version 0.18.0 is now available from:

  <https://qweercoincore.org/bin/qweercoin-core-0.18.0/>
...
Please report bugs using the issue tracker at GitHub:
  ...
  <https://github.com/qweercoin/qweercoin/issues>

To receive security and update notifications, please subscribe to:

  <https://qweercoincore.org/en/list/announcements/join/>
...
`/Applications/Qweercoin-Qt` (on Mac) or `qweercoind`/`qweercoin-qt` (on
Linux).
...
Qweercoin Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use Qweercoin Core on unsupported systems.
...
10.10. Additionally, Qweercoin Core does not yet change appearance when
...
`contrib/init/qweercoind.service`, it has been changed to use
`/var/lib/qweercoind` as the data directory instead of
`~qweercoin/.qweercoin`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/qweercoind` will
exist has enough space (check using `df -h /var/lib/qweercoind`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/qweercoin/qweercoin/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://qweercoincore.org/en/contact/) the Qweercoin Core
...
The systemd init file (`contrib/init/qweercoind.service`) has been changed
to use `/var/lib/qweercoind` as the data directory instead of
`~qweercoin/.qweercoin`. This change makes Qweercoin Core more consistent with
...
`datadir` option in `/etc/qweercoin/qweercoin.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/qweercoin/qweercoin.conf`.


Documentation
-------------

- A new short [document](https://github.com/qweercoin/qweercoin/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/qweercoin/qweercoin/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/qweercoin/qweercoin/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/qweercoin/qweercoin/blob/master/doc/qweercoin-conf.md)
  about the `qweercoin.conf` file describes how to use it to configure
  Qweercoin Core.

- A new document introduces Qweercoin Core's BIP174 [Partially-Signed
  Qweercoin Transactions
  (PSBT)](https://github.com/qweercoin/qweercoin/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/qweercoin/qweercoin/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Qweercoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Qweercoin Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/qweercoin/qweercoin/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/qweercoin/qweercoin/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Qweercoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Qweercoin Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/qweercoin/qweercoin/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/qweercoin/qweercoin/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart qweercoind with the `-deprecatedrpc=generate`
...
- A new `qweercoin-wallet` tool is now distributed alongside Qweercoin
...
This section describes planned changes to Qweercoin Core that may affect
other Qweercoin software and services.

- Since version 0.16.0, Qweercoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Qweercoin Core 0.20 (expected about
  a year after 0.18), Qweercoin Core will default to native segwit
  ...
  to bech32 addresses, and if the Qweercoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Qweercoin Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Qweercoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Qweercoin Core
...
- A sub-project of Qweercoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Qweercoin Core.  See their [project
  page](https://github.com/qweercoin-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Qweercoin Core's own implementation, although entropy
  gathered by Qweercoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Qweercoin Core a
...
- On macOS, Qweercoin Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `qweercoin_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libqweercoin_server.a` (jnewbery)
...
- #13926 [Tools] qweercoin-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like qweercoind (MarcoFalke)
...
- #14521 qt, docs: Fix `qweercoin-qt -version` output formatting (hebasto)
...
- #11625 Add QweercoinApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libqweercoin_server` (laanwj)
...
- #15154 Configure: qweercoin-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/qweercoin-wallet (Sjors)
...
- #11526 Visual Studio build configuration for Qweercoin Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libqweercoin_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in qweercoin-util-test.py (isghe)
...
- #12775 Integration of property based testing into Qweercoin Core (Christewart)
...
- #12246 Bugfix: Only run qweercoin-tx tests when qweercoin-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_qweercoin` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_qweercoin` (MarcoFalke)
...
- #14186 qweercoin-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_qweercoin.conf` message on startup if conf file exists (leishman)
...
- #12255 Update qweercoin.service to conform to init.md (dongcarl)
...
- #14497 Add doc/qweercoin-conf.md (hebasto)
...
- #15354 Add missing `qweercoin-wallet` tool manpages (MarcoFalke)
...
- #15765 correct qweercoinconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support qweercoin folder on OSX (JimmyMow)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/qweercoin/).
