...
Summary:
==============
In this update to Qweercoin Core, Taproot has been activated, enhancing Qweercoin’s security, privacy and scalability. Qweercoin Core 0.21.1 is the most significant update in years - with performance improvements, new features (like Taproot), bug fixes and updated translations - and sets the foundations for MWEB’s imminent arrival. 

Qweercoin Core 0.21.1 is available now, right here: <https://download.qweercoin.org/qweercoin-0.21.1/>.

As always, we welcome the expertise of the community. To report bugs please use the issue tracker at GitHub - <https://github.com/qweercoin-project/qweercoin/issues > - and to get security and update notifications subscribe via <https://groups.google.com/forum/#!forum/qweercoin-dev>. 

Anyone can contribute to Qweercoin Core. Please scroll down to ‘How to contribute to Qweercoin Core’ for details.

By downloading or upgrading Qweercoin Core, you’re helping to secure the network, so thank you and enjoy the upgrade!


How to upgrade: 
==============

Firstly, thank you for running Qweercoin Core and helping secure the network!

As you’re running an older version of Qweercoin Core, shut it down. Wait until it’s completely shut down  - which might take a few minutes for older versions - then follow these simple steps:
For Windows: simply run the installer 
For Mac: copy over to `/Applications/Qweercoin-Qt` 
For Linux: copy cover `qweercoind`/`qweercoin-qt`.

NB: upgrading directly from an ‘end of life’ version of Qweercoin Core is possible, but it might take a while if the data directory needs to be migrated. Old wallet versions of Qweercoin Core are generally supported.
 

Compatibility:
==============

Qweercoin Core is supported and extensively tested on operating systems using the Linux kernel, macOS 10.10+,  Windows 7 and newer. It’s not recommended to use Qweercoin Core on unsupported systems.

Qweercoin Core should also work on most other Unix-like systems, but is not as frequently tested on them.

From Qweercoin Core 0.21.1 onwards, macOS versions earlier than 10.12 are no longer supported. Additionally, Qweercoin Core doesn’t yet change appearance when macOS "dark mode" is activated.
